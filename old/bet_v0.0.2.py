...
class Table:
    ...
    def add_row (self, row = None):
        if type (row) == list: self._data.append (row)
        elif type (row) == tuple: self._data.append (list (row))
        elif row is not None:
            raise ValueError ('row ist weder Liste, noch Tuple')
        else: self._data.append ([])
        self.row += 1

    def add_data (self, data):
        if self.row >= 0:
            if type (data) in (list, tuple): self._data [self.row] += data
            else: self._data [self.row].append (data)
        else: raise TableError ('Tabelle enthält noch keine Reihe')
    ...
    def del_last_row (self):
        if self.row >= 0:
            self._data.pop()
            self.row -= 1
        else: raise TableError ('Tabelle enthält noch keine Reihe zum Löschen')
...
class TableRead (html.parser.HTMLParser):
    ...
    def tr_start (self, attrs):
        if self.table is not None: self.table.add_row()

    def td_start (self, attrs):
        if self.table is not None and not self.incell:
            for attr in attrs:
                if attr [0] == 'colspan': self.colspaned = True
            if not self.colspaned: self.incell = True

    def th_start (self, attrs):
        if self.table is not None and not self.incell:
            for attr in attrs:
                if attr [0] == 'colspan': self.colspaned = int (attr [1])
            self.incell = True

    def img (self, attrs):
        if self.incell: self.tmpdat += '_img_'
    ...
    def td_end (self):
        if self.table is not None: self.add_cell_data (self.table.add_data)
    ...
    def th_end (self):
        if self.table is not None:
            if type (self.colspaned) == int:
                ...
                self.tmpdat = ''
                self.incell = False
            else: self.add_cell_data (self.table.add_header_data)
    ...
    def add_cell_data (self, func):
        if self.incell and not self.colspaned:
            if type (self.tmpdat) == str:
                ...
                self.tmpdat = ''
                for word in tmplist: self.tmpdat += word + ' '
                self.tmpdat = self.tmpdat.strip()
            func (self.tmpdat)
            self.tmpdat = ''
            self.incell = False
    ...
    def handle_starttag (self, tag, attrs):
        if tag in self.starttags.keys(): self.starttags [tag] (attrs)

    def handle_endtag (self, tag):
        if tag in self.endtags.keys(): self.endtags [tag]()
    ...
    def handle_charref (self, name):
        if self.incell:
            try: self.tmpdat += chr (int (name))
            except ValueError: pass

    def handle_entityref (self, name):
        if self.incell: self.tmpdat += html.entities.entitydefs [name]
    ...
    with TableRead() as parser:
        ...
        while parser.getpos() [0] < linecount:
            try:
                parser.feed (page)
                if not parerr: break
            except html.parser.HTMLParseError: parerr = True
        return parser.tables

# with open ('nested.html', 'r') as f: page = f.read()
url = 'http://java.sun.com/javase/6/docs/api/overview-summary.html'
