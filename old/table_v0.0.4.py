...
class CmdWidget:
    ...
    def _setup_widgets (self):
        label = ttk.Label (self.frame, text = 'Adresse: ')
        tmp_frame = ttk.Frame (self.frame)
        entry = ttk.Entry (tmp_frame, width = 0)
        button = ttk.Button (tmp_frame, text = 'Öffnen')
...
if __name__ == '__main__':
    ...
    cmd = CmdWidget (root)
    cmd.frame.pack (fill = 'x', anchor = 'n')
    ...
    for table in tables:
        print (table)
        i += 1
        tw = TableWidget (nb, table)
        nb.add (tw.frame, text = 'Tabelle ' + str (i))
    nb.pack (expand = True, fill = 'both', anchor = 'n')
    root.mainloop()
